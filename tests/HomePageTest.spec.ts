  ...
  async () => {
    ...
    test("It can validate all links on the Home Page", async ({
      ...
      for (const link of await homePage.getAllLinksURL()) {
        ...
        test.step(
          "It can validate the status code for => " + link,
          async () => {
            await expect
    ...
    test("It can validate the header of the Home Page", async ({
      homePage,
    }) => {
      ...
      await test.step("It can validate header links visibility & clickability", async () => {
        ...
        await expect(
          await homePage.getLocator_signInLink_Header()
        ).toBeVisible();
        await expect(
          await homePage.getLocator_signInLink_Header()
        ).toBeEnabled();
